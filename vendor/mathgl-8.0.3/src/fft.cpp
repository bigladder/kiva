...
//	const double *c = (const double *)wt;
	...
	double c0=cos(2*M_PI/n), s0=sin(2*M_PI/n), c1=c0, s1=s0;
	memset(d,0,2*n*sizeof(double));
	if(inv)	for(long i=0;i<n;i++)
	{
		double cc=1, ss=0, tmp;
		for(long j=0;j<n;j++)
		{
			long ii = 2*i, jj = 2*j*s;
			d[ii] 	+= x[jj]*cc-x[jj+1]*ss;
			d[ii+1] += x[jj+1]*cc+x[jj]*ss;
			tmp = cc*c1-ss*s1;	ss = ss*c1+cc*s1;	cc = tmp;
		}
		tmp = c1*c0-s1*s0;	s1 = s1*c0+c1*s0;	c1 = tmp;
	}
...
#pragma omp for nowait schedule(static)
		for(long ii=t->id;ii<t->n;ii+=mglNumThr)
		{
			long i = ii%nx, k = (ny-1)*(ii/nx)-1;	memset(b,0,2*ny*sizeof(double));
			for(long j=1;j<ny;j++)	b[2*j]=sin(M_PI*j/ny)*(a[i+nx*(k+j)]+a[i+nx*(k+ny-j)])+(a[i+nx*(k+j)]-a[i+nx*(k+ny-j)])*0.5;
			mgl_fft(b,1,ny,t->v,w,false);
			// a[i+nx*k]=0;	
			a[i+nx*(k+1)]=b[0]*f/2;	// fill sinfft
			for(long j=1;j<ny/2;j++)
			{
				a[i+nx*(k+2*j)] = -b[2*j+1]*f;
				a[i+nx*(k+2*j+1)] = a[i+nx*(k+2*j-1)]+b[2*j]*f;
			}
			if(ny%2)	a[i+nx*(k+ny-1)] = -b[ny]*f;
		}
		mgl_fft_free_thr(w);	delete []b;
	}
	return 0;
...
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0],ny=t->p[1],nz=t->p[2]+1,k=nx*ny;
	mreal *a = (mreal*)t->a;
#if !MGL_HAVE_PTHREAD
#pragma omp parallel
#endif
...
#pragma omp for nowait schedule(static)
		for(long i=t->id;i<t->n;i+=mglNumThr)
		{
			...
			long ii = i-k;
			for(long j=1;j<nz;j++)	b[2*j]=sin(M_PI*j/nz)*(a[ii+k*j]+a[ii+k*(nz-j)])+(a[ii+k*j]-a[ii+k*(nz-j)])*0.5;
			mgl_fft(b,1,nz,t->v,w,false);
			// a[ii]=0;	
			a[ii+k]=b[0]*f/2;	// fill sinfft
			for(long j=1;j<nz/2;j++)
			{
				a[ii+k*2*j] = -b[2*j+1]*f;
				a[ii+k*(2*j+1)] = a[ii+k*(2*j-1)]+b[2*j]*f;
			}
			if(nz%2)	a[ii+k*nz-k] = -b[nz]*f;
		}
		mgl_fft_free_thr(w);	delete []b;
	}
	return 0;
...
//-----------------------------------------------------------------------------
static void* mgl_sinxc(void *par)
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0]+1;
	dual *a = (dual*)t->a;
...
#endif
	{
		double *b = new double[2*nx], f=sqrt(0.5/nx);
		void *w = mgl_fft_alloc_thr(nx);
#pragma omp for nowait schedule(static)
		for(long i=t->id;i<t->n;i+=mglNumThr)
		{
			...
			for(long j=1;j<nx;j++)
			{
				long i1 = j+k, i2 = nx-j+k;
				double s = sin(M_PI*j/nx);
				b[2*j]   = s*real(a[i1]+a[i2])+real(a[i1]-a[i2])*0.5;
				b[2*j+1] = s*imag(a[i1]+a[i2])+imag(a[i1]-a[i2])*0.5;
			}
			mgl_fft(b,1,nx,t->v,w,false);
			a[k+1]=dual(b[0],b[1])*f;	// fill sinfft
			for(long j=1;j<nx/2;j++)
			{
				long jj = k+2*j;
				a[jj] = dual(b[2*(nx-j)+1]-b[2*j+1], b[2*j]-b[2*(nx-j)])*f;
				a[jj+1] = a[jj-1]+dual(b[2*j]+b[2*(nx-j)], b[2*j+1]+b[2*(nx-j)+1])*f;
			}
			if(nx%2)	a[k+nx-1] = dual(b[nx+2]-b[nx], b[nx-1]-b[nx+1])*f;
		}
		mgl_fft_free_thr(w);	delete []b;
	}
	return 0;
}
static void* mgl_sinyc(void *par)
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0],ny=t->p[1]+1;
	dual *a = (dual*)t->a;
...
#endif
	{
		double *b = new double[2*ny], f=sqrt(0.5/ny);
		void *w = mgl_fft_alloc_thr(ny);
#pragma omp for nowait schedule(static)
		for(long ii=t->id;ii<t->n;ii+=mglNumThr)
		{
			...
			for(long j=1;j<ny;j++)
			{
				long i1 = i+nx*(k+j), i2 = i+nx*(k+ny-j);
				double s = sin(M_PI*j/ny);
				b[2*j]   = s*real(a[i1]+a[i2])+real(a[i1]-a[i2])*0.5;
				b[2*j+1] = s*imag(a[i1]+a[i2])+imag(a[i1]-a[i2])*0.5;
			}
			mgl_fft(b,1,ny,t->v,w,false);
			// a[i+nx*k]=0;	
			a[i+nx*(k+1)]=dual(b[0],b[1])*f;	// fill sinfft
			for(long j=1;j<ny/2;j++)
			{
				long jj = i+nx*(k+2*j);
				a[jj] = dual(b[2*(ny-j)+1]-b[2*j+1], b[2*j]-b[2*(ny-j)])*f;
				a[jj+nx] = a[jj-nx]+dual(b[2*j]+b[2*(ny-j)], b[2*j+1]+b[2*(ny-j)+1])*f;
			}
			if(ny%2)	a[i+nx*(k+ny-1)] = dual(b[ny+2]-b[ny], b[ny-1]-b[ny+1])*f;
		}
		mgl_fft_free_thr(w);	delete []b;
	}
	return 0;
}
static void* mgl_sinzc(void *par)
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0],ny=t->p[1],nz=t->p[2]+1,k=nx*ny;
	dual *a = (dual*)t->a;
...
#endif
	{
		double *b = new double[2*nz], f=sqrt(0.5/nz);
		void *w = mgl_fft_alloc_thr(nz);
#pragma omp for nowait schedule(static)
		for(long i=t->id;i<t->n;i+=mglNumThr)
		{
			...
			for(long j=1;j<nz;j++)
			{
				long i1 = ii+k*j, i2 = ii+k*(nz-j);
				double s = sin(M_PI*j/nz);
				b[2*j]   = s*real(a[i1]+a[i2])+real(a[i1]-a[i2])*0.5;
				b[2*j+1] = s*imag(a[i1]+a[i2])+imag(a[i1]-a[i2])*0.5;
			}
			mgl_fft(b,1,nz,t->v,w,false);
			// a[ii]=0;	
			a[ii+k]=dual(b[0],b[1])*f;	// fill sinfft
			for(long j=1;j<nz/2;j++)
			{
				long jj = ii+k*2*j;
				a[jj] = dual(b[2*(nz-j)+1]-b[2*j+1], b[2*j]-b[2*(nz-j)])*f;
				a[jj+k] = a[jj-k]+dual(b[2*j]+b[2*(nz-j)], b[2*j+1]+b[2*(nz-j)+1])*f;
			}
			if(nz%2)	a[ii+k*nz-k] = dual(b[nz+2]-b[nz], b[nz-1]-b[nz+1])*f;
		}
		mgl_fft_free_thr(w);	delete []b;
	}
	return 0;
...
void MGL_EXPORT mgl_datac_sinfft(HADT c, const char *dir)
{
	...
	bool clear=false;
	void *wt=0;
	long nx=c->nx, ny=c->ny, nz=c->nz, par[3]={nx,ny,nz};
	if(strchr(dir,'x') && nx>1)
	{
		if(mgl_fft_data.wnx==nx+1)	wt = mgl_fft_data.wtx;
		else	{	clear = true;	wt = mgl_fft_alloc(nx+1,0,0);	}
		mglStartThreadT(mgl_sinxc,ny*nz,c->a,0,wt,0,par);
		if(mgl_fft_data.wnx==0)
		{	mgl_fft_data.wtx = wt;	clear = false;	mgl_fft_data.wnx=nx+1;	}
		if(clear)	{	mgl_fft_free(wt,0,0);	clear = false;	}
	}
	if(strchr(dir,'y') && ny>1)
	{
		...
		mglStartThreadT(mgl_sinyc,nx*nz,c->a,0,wt,0,par);
		if(mgl_fft_data.wny==0)
		{	mgl_fft_data.wty = wt;	clear = false;	mgl_fft_data.wny=ny+1;	}
		if(clear)	{	mgl_fft_free(wt,0,0);	clear = false;	}
	}
	if(strchr(dir,'z') && nz>1)
	{
		...
		mglStartThreadT(mgl_sinzc,nx*ny,c->a,0,wt,0,par);
		if(mgl_fft_data.wnz==0)
		{	mgl_fft_data.wtz = wt;	clear = false;	mgl_fft_data.wnz=nz+1;	}
		if(clear)	{	mgl_fft_free(wt,0,0);	clear = false;	}
	}
...
#pragma omp for nowait schedule(static)
		for(long ii=t->id;ii<t->n;ii+=mglNumThr)
		{
			...
			mgl_fft(b,1,nn,t->v,w,false);
			const long i0 = i+nx*(ny*k+nn), i1 = i+nx*ny*k;
			double f1=0.5*(a[i1]-a[i0]), s=-1;
			a[i0]=0.5*(a[i1]+a[i0]*((nn%2)?-1:1));
			for(long j=1;j<nn;j++)
			{
				long ii = i+nx*(ny*k+j);
				f1 += a[ii]*cos(M_PI*j/nn);
				a[i0] += a[ii]*s;	s = -s;
			}
			a[i1]=b[0]*f;	a[i+nx*(ny*k+1)]=f1*f;	a[i0]*=f;	// fill cosfft
			for(long j=1;j<nn/2;j++)
			{
				long ii = i+nx*(ny*k+2*j);
				a[ii] = b[2*j]*f;
				a[ii+nx] = a[ii-nx]-b[2*j+1]*f;
			}
...
//-----------------------------------------------------------------------------
static void* mgl_cosxc(void *par)
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0],nn=nx-1;
	dual *a = (dual*)t->a;
...
#endif
	{
		double *b = new double[2*nx], f=sqrt(0.5/nn);
		void *w = mgl_fft_alloc_thr(nn);
#pragma omp for nowait schedule(static)
		for(long i=t->id;i<t->n;i+=mglNumThr)
		{
			...
			for(long j=0;j<nn;j++)
			{
				double s = sin(M_PI*j/nn);
				long i1 = j+k, i2 = nn-j+k;
				b[2*j]  = real(a[i1]+a[i2])*0.5-s*real(a[i1]-a[i2]);
				b[2*j+1]= imag(a[i1]+a[i2])*0.5-s*imag(a[i1]-a[i2]);
			}
			mgl_fft(b,1,nn,t->v,w,false);
			dual f1=0.5*(a[k]-a[nn+k]), s=-1;
			a[nn+k]=0.5*(a[k]+a[nn+k]*((nn%2)?-1.:1.));
			for(long j=1;j<nn;j++)
			{
				f1 += a[j+k]*cos(M_PI*j/nn);
			...
			a[k]=dual(b[0],b[1])*(f*2);
			a[1+k]=f1*(f*2);	a[nn+k]*=(f*2);	// fill cosfft
			for(long j=1;j<nn/2;j++)
			{
				long jj = 2*j+k;
				a[jj] = dual(b[2*j]+b[2*(nn-j)], b[2*j+1]+b[2*(nn-j)+1])*f;
				a[jj+1] = a[jj-1]+dual(b[2*(nn-j)+1]-b[2*j+1], b[2*j]-b[2*(nn-j)])*f;
			}
			if(nn%2)	a[nn-1+k] = dual(b[nn-1]+b[nn+1], b[nn]+b[nn+2])*f;
		}
		mgl_fft_free_thr(w);
		delete []b;
	}
...
static void* mgl_cosyc(void *par)
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0],ny=t->p[1],nn=ny-1;
	dual *a = (dual*)t->a;
...
#endif
	{
		double *b = new double[2*ny], f=sqrt(0.5/nn);
		void *w = mgl_fft_alloc_thr(nn);
#pragma omp for nowait schedule(static)
		for(long ii=t->id;ii<t->n;ii+=mglNumThr)
		{
			...
			for(long j=0;j<nn;j++)
			{
				long i1 = i+nx*(ny*k+j), i2 = i+nx*(ny*k+nn-j);
				double s = sin(M_PI*j/nn);
				b[2*j]   = real(a[i1]+a[i2])*0.5-s*real(a[i1]-a[i2]);
				b[2*j+1] = imag(a[i1]+a[i2])*0.5-s*imag(a[i1]-a[i2]);
			}
			mgl_fft(b,1,nn,t->v,w,false);
			const long i0 = i+nx*ny*k, i1 = i+nx*(ny*k+nn);
			dual f1=0.5*(a[i0]-a[i1]), s=-1;
			a[i1]=0.5*(a[i0]+a[i1]*((nn%2)?-1.:1.));
			for(long j=1;j<nn;j++)
			{
				long jj = i+nx*(ny*k+j);
				f1 += a[jj]*cos(M_PI*j/nn);
				a[i1] += a[jj]*s;	s = -s;
			}
			a[i0]=dual(b[0],b[1])*(f*2);
			a[i+nx*(ny*k+1)]=f1*(f*2);	a[i1]*=(f*2);	// fill cosfft
			for(long j=1;j<nn/2;j++)
			{
				long jj = i+nx*(ny*k+2*j);
				a[jj] = dual(b[2*j]+b[2*(nn-j)], b[2*j+1]+b[2*(nn-j)+1])*f;
				a[jj+nx] = a[jj-nx]+dual(b[2*(nn-j)+1]-b[2*j+1], b[2*j]-b[2*(nn-j)])*f;
			}
			if(nn%2)	a[i+nx*(ny*k+nn-1)] = dual(b[nn-1]+b[nn+1], b[nn]+b[nn+2])*f;
		}
		mgl_fft_free_thr(w);
		delete []b;
	}
...
static void* mgl_coszc(void *par)
{
	mglThreadT *t=(mglThreadT *)par;
	long nx=t->p[0],ny=t->p[1],nz=t->p[2],k=nx*ny,nn=nz-1;
	dual *a = (dual*)t->a;
...
#endif
	{
		double *b = new double[2*nz], f=sqrt(0.5/nn);
		void *w = mgl_fft_alloc_thr(nn);
#pragma omp for nowait schedule(static)
		for(long i=t->id;i<t->n;i+=mglNumThr)
		{
			...
			for(long j=0;j<nn;j++)
			{
				long i1 = i+k*j, i2 = i+k*(nn-j);
				double s = sin(M_PI*j/nn);
				b[2*j]   = real(a[i1]+a[i2])*0.5-s*real(a[i1]-a[i2]);
				b[2*j+1] = imag(a[i1]+a[i2])*0.5-s*imag(a[i1]-a[i2]);
			}
			mgl_fft(b,1,nn,t->v,w,false);
			const long i1 = i+k*nn;
			dual f1=0.5*(a[i]-a[i1]), s=-1;
			a[i1]=0.5*(a[i]+a[i1]*((nn%2)?-1.:1.));
			for(long j=1;j<nn;j++)
			{
				f1 += a[i+k*j]*cos(M_PI*j/nn);
				a[i1] += a[i+k*j]*s;	s = -s;
			}
			a[i]=dual(b[0],b[1])*(f*2);	
			a[i+k]=f1*(f*2);	a[i1]*=(f*2);	// fill cosfft
			for(long j=1;j<nn/2;j++)
			{
				long jj = i+k*2*j;
				a[jj] = dual(b[2*j]+b[2*(nn-j)], b[2*j+1]+b[2*(nn-j)+1])*f;
				a[jj+k] = a[jj-k]+dual(b[2*(nn-j)+1]-b[2*j+1], b[2*j]-b[2*(nn-j)])*f;
			}
			if(nn%2)	a[i+k*nn-k] = dual(b[nn-1]+b[nn+1], b[nn]+b[nn+2])*f;
		}
		mgl_fft_free_thr(w);
		delete []b;
	}
...
void MGL_EXPORT mgl_datac_cosfft(HADT c, const char *dir)
{
	...
	if(strchr(dir,'x') && nx>1)
	{
		...
		mglStartThreadT(mgl_cosxc,ny*nz,c->a,0,wt,0,par);
		if(mgl_fft_data.wnx==0)
		{	mgl_fft_data.wtx = wt;	clear = false;	mgl_fft_data.wnx=nx-1;	}
		if(clear)	{	mgl_fft_free(wt,0,0);	clear = false;	}
	}
	if(strchr(dir,'y') && ny>1)
	{
		...
		mglStartThreadT(mgl_cosyc,nx*nz,c->a,0,wt,0,par);
		if(mgl_fft_data.wny==0)
		{	mgl_fft_data.wty = wt;	clear = false;	mgl_fft_data.wny=ny-1;	}
		if(clear)	{	mgl_fft_free(wt,0,0);	clear = false;	}
	}
	if(strchr(dir,'z') && nz>1)
	{
		...
		mglStartThreadT(mgl_coszc,nx*ny,c->a,0,wt,0,par);
		if(mgl_fft_data.wnz==0)
		{	mgl_fft_data.wtz = wt;	clear = false;	mgl_fft_data.wnz=nz-1;	}
		if(clear)	{	mgl_fft_free(wt,0,0);	clear = false;	}
	}
