...
int static mgls_cosfft(mglGraph *, long , mglArg *a, const char *k, const char *)
{
	...
	else if(c && !strcmp(k,"ds"))	c->CosFFT(a[1].s.s);
	else res = 1;
	return res;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
int static mgls_histl(mglGraph *gr, long , mglArg *a, const char *k, const char *opt)
{
	int res=0;
	if(k[0]=='d' && a[0].d->temp)	return 5;
	mglData *d = dynamic_cast<mglData *>(a[0].d);
	if(!d)	return 1;
	if(!strcmp(k,"ddnnn"))	*d = mglData(true,mgl_data_hist_l(a[1].d,NULL,int(a[2].v+0.5), a[3].v, a[4].v));
	else if(!strcmp(k,"dddnnn"))	*d = mglData(true,mgl_data_hist_l(a[1].d,a[2].d, mgl_int(a[3].v), a[4].v, a[5].v));
	else res = 1;
	return res;
}
//-----------------------------------------------------------------------------
...
int static mgls_ode(mglGraph *, long , mglArg *a, const char *k, const char *)
{
	...
	if(d)
	{
		...
		else if(!strcmp(k,"dssdnns"))
			*d = mglODE(a[1].s.s, a[2].s.s, *(a[3].d), a[4].v, a[5].v, a[6].s.s);
		if(!strcmp(k,"dsssd"))
	...
	else if(c)
	{
		if(!strcmp(k,"dssd"))
			...
			*c = mglODEc(a[1].s.s, a[2].s.s, *(a[3].d), a[4].v, a[5].v, a[6].s.s);
		if(!strcmp(k,"dsssd"))
...
int static mgls_sinfft(mglGraph *, long , mglArg *a, const char *k, const char *)
{
	...
	else if(c && !strcmp(k,"ds"))	c->SinFFT(a[1].s.s);
	else res = 1;
	return res;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
int static mgls_mode(mglGraph *, long , mglArg *a, const char *k, const char *)
{
	int res=0;
	if(k[0]=='d' && a[0].d->temp)	return 5;
	mglData *d = dynamic_cast<mglData *>(a[0].d);
	mglDataC *c = dynamic_cast<mglDataC *>(a[0].d);
	if(d && !strcmp(k,"ddsn"))	d->Mode(*(a[1].d), a[2].s.s, a[3].v);
	else if(d && !strcmp(k,"ddsnn"))	d->Mode(*(a[1].d), a[2].s.s, a[3].v, a[4].v);
	else if(d && !strcmp(k,"dddsn"))	d->Mode(*(a[1].d), *(a[2].d), a[3].s.s, a[4].v);
	else if(d && !strcmp(k,"dddsnn"))	d->Mode(*(a[1].d), *(a[2].d), a[3].s.s, a[4].v, a[5].v);
	else if(c && !strcmp(k,"ddsn"))		c->Mode(*(a[1].d), a[2].s.s, a[3].v);
	else if(c && !strcmp(k,"ddsnn"))	c->Mode(*(a[1].d), a[2].s.s, a[3].v, a[4].v);
	else if(c && !strcmp(k,"dddsn"))	c->Mode(*(a[1].d), *(a[2].d), a[3].s.s, a[4].v);
	else if(c && !strcmp(k,"dddsnn"))	c->Mode(*(a[1].d), *(a[2].d), a[3].s.s, a[4].v, a[5].v);
	else res = 1;
	return res;
}
//-----------------------------------------------------------------------------
...
mglCommand mgls_dat_cmd[] = {
	...
	{"histl",_("Create histogram of data values using linear interpolation"),"histl Res Dat num v1 v2|Res Dat Wdat num v1 v2", mgls_histl ,4},
	...
	{"mode",_("Find mode for given ini"),"mode Dat Mask 'how' kdx [iter]|Dat Mask Pot 'how' kdx [iter]", mgls_mode ,16},
	...
	{"ode",_("Solve ODE"),"ode Res 'df' 'var' Ini [dt tmax 'refl']|Res 'df' 'var' 'brd' Ini [dt tmax]", mgls_ode ,4},
