...
//#pragma omp parallel for collapse(2)
	...
	{
		HMDT d = mglFormulaCalc(p->str[j].c_str(),p->head);
		mreal val = d->a[0];
		if(d->nx<n)
		{
			d->Create(n);
			for(long i=0;i<n;i++)	d->a[i] = val;
		}
		else switch(p->brd)
		{
			default:	// zero instead of NAN
			case 0:	
			case 'z':
			case '0':
		...
		memcpy(dx+j*n, d->a, n*sizeof(mreal));
		delete d;
	}
}
HMDT MGL_EXPORT mgl_ode_solve_set(const char *func, const char *var, char brd, HCDT x0, mreal dt, mreal tmax)
{
	...
	(dat+m+2)->Create(nn);	(dat+m+2)->s = "#$mgl";
	par.head.push_back(dat+m+2);
	...
	delete []xx;	delete []dat;	return res;
}
//-----------------------------------------------------------------------------
...
//#pragma omp parallel for collapse(2)
	for(long j=0;j<p->m;j++)
	{
		HADT d = mglFormulaCalcC(p->str[j].c_str(),p->head);
		dual val = d->a[0];
		if(d->nx<n)
		{
			d->Create(n);
		...
		memcpy(dx+2*j*n, d->a, 2*n*sizeof(mreal));
		delete d;
	}
}
HADT MGL_EXPORT mgl_ode_solve_set_c(const char *func, const char *var, char brd, HCDT x0, mreal dt, mreal tmax)
{
	...
	HADT dat = new mglDataC[m];
	for(long i=0;i<m;i++)
	{
		HADT d = dat+i;	d->Create(nn);
		d->s = var[i];	par.head.push_back(d);
	}
	HMDT d = new mglData[3];
	d->Create(nn);	d->s = 'j';
	for(long i=0;i<nn;i++)	d->a[i] = i;
	par.head.push_back(d);
	(d+1)->Create(nn);	(d+1)->s = "#$mgl";
	par.head.push_back(d+1);
	par.t = d+2;	par.t->s = 't';
	par.head.push_back(par.t);

	mreal *xx = new mreal[2*n];
	const mglDataC *c = dynamic_cast<const mglDataC *>(x0);
	if(c)		for(long i=0;i<n;i++)	
	{	xx[2*i]=real(c->a[i]);	xx[2*i+1]=imag(c->a[i]);	}
	else if(x0)	for(long i=0;i<n;i++)	
	{	xx[2*i] = x0->vthr(i);	xx[2*i+1]=0;	}
...
//-----------------------------------------------------------------------------
void MGL_NO_EXPORT mgl_txt_bord(mreal *x, const mreal *, void *par)
{
	mglEqTxT *p=(mglEqTxT *)par;
	mreal vars[MGL_VS];
	const long n = p->n;
...
//#pragma omp parallel for
	for(long i=0;i<n;i++)
		x[i] = mgl_expr_eval_v(p->eqRB[i], vars);
}
HMDT MGL_EXPORT mgl_ode_solve_str_b(const char *func, const char *var, HCDT x0, mreal dt, mreal tmax, const char *bord)
{
	if(!var || !(*var) || !func)	return 0;
	mglEqTxT par;
	par.var=var;	par.FillReal(func, bord);
	long n = par.n = long(par.str.size());
	mreal *xx = new mreal[n];
	for(long i=0;i<n;i++)	xx[i] = x0?x0->vthr(i):0;
	HMDT res = mgl_ode_solve_ex(mgl_txt_func,n,xx,dt,tmax,&par,(bord && *bord)?mgl_txt_bord:NULL);
	delete []xx;	return res;
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void MGL_NO_EXPORT mgl_txt_bordC(mreal *x, const mreal *, void *par)
{
	mglEqTxT *p=(mglEqTxT *)par;
	mdual vars[MGL_VS];
	const long n = p->n;
...
//#pragma omp parallel for
	for(long i=0;i<n;i++)
	{
		dual r = mgl_cexpr_eval_v(p->eqCB[i], vars);
		x[2*i] = real(r);	x[2*i+1] = imag(r);
	}
}
HADT MGL_EXPORT mgl_ode_solve_str_cb(const char *func, const char *var, HCDT x0, mreal dt, mreal tmax, const char *jump)
{
	if(!var || !(*var) || !func)	return 0;
	mglEqTxT par;	par.var=var;
	par.var=var;	par.FillCmplx(func, jump);
	long n = par.n = long(par.str.size());
	mreal *xx = new mreal[2*n];
	const mglDataC *c = dynamic_cast<const mglDataC *>(x0);
	for(long i=0;i<n;i++)
	...
	HMDT res = mgl_ode_solve_ex(mgl_txt_funcC,2*n,xx,dt,tmax,&par,(jump && *jump)?mgl_txt_bordC:NULL);
	delete []xx;
	const long nt=res->ny;
	mglDataC *out = new mglDataC(n, nt);
#pragma omp parallel for schedule(static)
...
HMDT MGL_EXPORT mgl_ode_solve_ex(void (*func)(const mreal *x, mreal *dx, void *par), int n, const mreal *x0, mreal dt, mreal tmax, void *par, void (*jump)(mreal *x, const mreal *xp, void *par))
{
	...
	mreal *x=new mreal[n+1], *k1=new mreal[n], *k2=new mreal[n], *k3=new mreal[n], *v=new mreal[n+1], hh=dt/2;
	// initial conditions
	...
	for(k=1;k<nt && good;k++)
	{
		...
		if(jump)	jump(x,res->a+n*(k-1),par);
		for(long i=0;i<n;i++)
