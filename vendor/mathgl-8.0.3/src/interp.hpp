...
struct mglEqTxT
{
	...
	void FillStr(const char *eqs, const char *bds=NULL)
	{
		const char *f=eqs;
		while(1)
		{
		...
		if(bds && *bds)
		{
			...
			{
				const char *g = strchr(f,';');
				if(g)	bnd.push_back(std::string(f,g-f));
				else	{	bnd.push_back(f);	break;	}
				f = g+1;
			}
		}
		else	bnd.clear();
	}
	void FillReal(const char *eqs, const char *bds=NULL)
	{
		FillStr(eqs, bds);
		size_t n = str.size();	if(n==0)	return;
		eqR = new HMEX[n];
		for(size_t i=0;i<n;i++)	eqR[i] = mgl_create_expr(str[i].c_str());
		n = bnd.size();	if(n==0)	return;
		eqRB = new HMEX[n];
		for(size_t i=0;i<n;i++)	eqRB[i] = mgl_create_expr(bnd[i].c_str());
	}
	void FillCmplx(const char *eqs, const char *bds=NULL)
	{
		FillStr(eqs, bds);
		size_t n = str.size();	if(n==0)	return;
		eqC = new HAEX[n];
		...
		n = bnd.size();	if(n==0)	return;
		eqCB = new HAEX[n];
		for(size_t i=0;i<n;i++)	eqCB[i] = mgl_create_cexpr(bnd[i].c_str());
	}
};
//-----------------------------------------------------------------------------
#endif
