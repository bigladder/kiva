...
bool SetupDialog::convert()
{
	...
	for(i=0;i<10;i++)	// set light sources
	{
		...
		if(blight[i]->text().isEmpty())
			s=QString("light %1 %2 %3 %4 '%5'\n").arg(i).arg(x1).arg(y1).arg(z1).arg(col[j].toLatin1());
		else
			s=QString("light %1 %2 %3 %4 '%5' %g\n").arg(i).arg(x1).arg(y1).arg(z1).arg(col[j].toLatin1(), blight[i]->text().toDouble());
		res += s;
	}
	...
	{	s=QString("axis %1 %2 %3 %4 %5 %6\n").arg(x1).arg(y1).arg(z1).arg(x2).arg(y2).arg(z2);	res += s;	}
	else
	{
		if(u1 && u2)	{s=QString("xrange %1 %2\n").arg(x1).arg(x2);	res += s;}
		if(v1 && v2)	{s=QString("yrange %1 %2\n").arg(y1).arg(y2);	res += s;}
		if(w1 && w2)	{s=QString("zrange %1 %2\n").arg(z1).arg(z2);	res += s;}
	}
	u1 = !cmin->text().isEmpty();	if(u1)	x1 = cmin->text().toDouble();
	u2 = !cmax->text().isEmpty();	if(u2)	x2 = cmax->text().toDouble();
	if(u1&&u2)	{s=QString("crange %1 %2\n").arg(x1).arg(x2);	res += s;}
	...
	if(u1&&v1&&w1)	{s=QString("origin %1 %2 %3\n").arg(x1).arg(y1).arg(z1);	res += s;}
	...
	if(u1 && u2)
	{
		if(xort->text().isEmpty())	s=QString("xtick %1 %2\n").arg(x1).arg(x2);
		else	s=QString("xtick %1 %2 %3\n").arg(x1).arg(x2).arg(xort->text().toDouble());
		res += s;
	}
	if(v1 && v2)
	{
		if(yort->text().isEmpty())	s=QString("ytick %1 %2\n").arg(y1).arg(y2);
		else	s=QString("ytick %1 %2 %3\n").arg(y1).arg(y2).arg(yort->text().toDouble());
		res += s;
	}
	if(w1 && w2)
	{
		if(zort->text().isEmpty())	s=QString("ztick %1 %2\n").arg(z1).arg(z2);
		else	s=QString("ztick %1 %2 %3\n").arg(z1).arg(z2).arg(zort->text().toDouble());
		res += s;
	}
	if(u1 && !u2)	{s=QString("xtick %1\n").arg(x1);	res += s;}
	if(v1 && !v2)	{s=QString("ytick %1\n").arg(y1);	res += s;}
	if(w1 && !w2)	{s=QString("ztick %1\n").arg(z1);	res += s;}
	...
	{
		s=QString("' %d\n").arg(xpos->currentIndex()-1);
		res = res + "xlabel '"+ xlbl->text() + s;
	}
	if(!ylbl->text().isEmpty())
	{
		s=QString("' %d\n").arg(ypos->currentIndex()-1);
		res = res + "ylabel '"+ ylbl->text() + s;
	}
	if(!zlbl->text().isEmpty())
	{
		s=QString("' %d\n").arg(zpos->currentIndex()-1);
		res = res + "zlabel '"+ zlbl->text() + s;
	}

	if(!aldef->text().isEmpty())
	{	s=QString("alphadef %1\n").arg(aldef->text().toDouble());	res += s;	}
	if(!amb->text().isEmpty())
	{	s=QString("ambient %1\n").arg(amb->text().toDouble());		res += s;	}

	if(!basew->text().isEmpty())
	{	s=QString("baselinewidth %1\n").arg(basew->text().toDouble());	res += s;	}
	if(!mesh->text().isEmpty())
	{	s=QString("meshnum %1\n").arg(mesh->text().toInt());	res += s;	}
	if(axial->currentIndex()>0)
	{
		s=QString("axialdir '%1'\n").arg(char('x'+axial->currentIndex()-1));
		res += s;
	}
	...
	if(!font->text().isEmpty())
	{
		...
		if(!fsize->text().isEmpty())	s=QString("' %1\n").arg(fsize->text().toDouble());
		else	s = QString("'\n");
		res += s;
	}
