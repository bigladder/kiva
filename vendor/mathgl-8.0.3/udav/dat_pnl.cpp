...
void DatPanel::refresh()
{
	...
	if(cc)	for(long i=0;i<nx;i++)	for(long j=0;j<ny;j++)
	{
		...
		else if(imag(f)>0)	s=QString("%1+%2i").arg(real(f)).arg(imag(f));
		else if(imag(f)<0)	s=QString("%1-%2i").arg(real(f)).arg(-imag(f));
		else	s= QString("%1").arg(real(f));
	...
	for(long i=0;i<m;i++)	ss[i] = QChar(vs[i]);
	s = QString(ss, m);	delete []ss;
	d=QString("%1 * %2 * %3").arg(nx).arg(ny).arg(nz);
	ready = true;
}
//-----------------------------------------------------------------------------
...
void DatPanel::putValue(int r, int c)
{
	...
	if(cc)
	{
		if(g!=cc->a[c+nx*(r+ny*kz)])
		{
			...
			else if(imag(g)>0)	s=QString("%1+%2i").arg(real(g)).arg(imag(g));
			else if(imag(g)<0)	s=QString("%2-%2i").arg(real(g)).arg(-imag(g));
			else	s=QString("%1").arg(real(g));
...
{
	QString mx=QString("%1").arg(nx), my=QString("%1").arg(ny), mz=QString("%1").arg(nz);
