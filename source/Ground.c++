			...
			for (size_t i = 0; i < nX; i++)
			{
				...
				default:
					...
					else
					{
						...
						Amat(i + nX*k, i + nX*k) = (1.0 + f*(A + C + E - B - D - F));
						Amat(i + nX*k, (i-1) + nX*k) = f*(B + D);
						Amat(i + nX*k, (i+1) + nX*k) = f*(-A - C);
						Amat(i + nX*k, i + nX*(k-1)) = f*F;
						Amat(i + nX*k, i + nX*(k+1)) = f*(-E);

						b(i + nX*k) = TOld[i][j][k]*(1.0 + (1-f)*(B + D + F - A - C - E))
